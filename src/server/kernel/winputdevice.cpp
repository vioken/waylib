...
public:
    ...
    QPointer<QObject> lastExclusiveGrabber;
    WSeat *seat = nullptr;
};
...
void WInputDevice::setExclusiveGrabber(QObject *grabber)
{
    ...
    d->lastExclusiveGrabber = firstPoint.exclusiveGrabber;
    dd->setExclusiveGrabber(nullptr, firstPoint.eventPoint, grabber);
}

QObject *WInputDevice::currentExclusiveGrabber() const
{
    W_DC(WInputDevice);
    auto pointerDevice = qobject_cast<QPointingDevice*>(d->qtDevice);
    if (!pointerDevice)
...
QObject *WInputDevice::lastExclusiveGrabber() const
{
    W_DC(WInputDevice);
    return d->lastExclusiveGrabber;
}
